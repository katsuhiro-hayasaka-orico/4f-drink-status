...
/**
 * 「スマホでも見られます」— shown on desktop widths only (CSS hides it below
 * 720px). The board is often left open on a PC or wall display, which makes
 * the page itself the best poster the site will ever get: a standing QR
 * right where people already look means anyone can pull the board onto
 * their phone without asking around for the URL.
 */
...
export function MobileInvite({ onShowQr }: MobileInviteProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    // 240px render shown at 120 CSS px — sharp enough to scan off a monitor.
    qrDataUrl(url, 240)
      .then((u) => {
